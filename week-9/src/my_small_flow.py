...
class SampleRegressionFlow(FlowSpec):
    """
    Sample regression flow is a minimal DAG showcasing reading data from a file
    ...
    def start(self):
        ...
        """
        #print("Starting up at {}".format(datetime.now(datetime.timezone.utc)))
        #debug printing this is from https://docs.metaflow.org/metaflow/tagging
        #to show how information about current run can be accessed programmatically
    ...
    def load_data(self):
        """
        Read the data on from the static file
        """
        from io import StringIO

        raw_data = StringIO(self.DATA_FILE).readlines()
        print("Total of {} rows in dataset".format(len(raw_data)))
    ...
    def train_model(self):
        """
        Train a regression model on training set
        """
    ...
    @step
    def test_model(self):
        """Test the model on hold out sample"""
        ...
        #all is done, lets move to the end
        self.next(self.end)
